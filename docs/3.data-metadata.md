...
- `@Listable(['field1', 'field2'])`: when using getters, tells Data API to retrieve 'field1' and 'field2' from the database instead of using the class field name.
...
export class MyDataAPI extends DataController(
  MyTable,
  {
    ...
    searchList: DefaultRoutes.WithOptions(DefaultRoutes.List, { pluckMode: 'search' }),
    detailedList: DefaultRoutes.WithOptions(DefaultRoutes.List, { pluckMode: 'detailed' }),
  },
  Controller('/my-endpoint')
) {
...
- **Compact list for search**:
  ...
  declare article: string;
  ```
  → The endpoint `/my-endpoint/searchList` will only return `article` and `price`.
  Example response:
  ```json
  [
    { "article": "Item A"},
...
- **Detailed view**:
  ...
  declare price: number;
  ```
  → The endpoint `/my-endpoint/detailedList` will return `_id`, `article`, and `price`.
  Example response:
  ```json
  [
...
- **Default view**:
  ...
  declare price: number;
  ```
  → The endpoint `/my-endpoint/list` will return only `article` and `price`.
  Example response:
  ```json
  [
...
- **Result for `/my-endpoint/searchList`**
```json
[
  { "article": "Item A" },
  { "article": "Item B" }
...
- **Result for `/my-endpoint/detailedList`**
```json
[
  { "_id": "1", "article": "Item A", "price": 10 },
  { "_id": "2", "article": "Item B", "price": 15 }
...
This decorator marks the field which will provide a reference to the database instance.
Please note that the presence of this field is *mandatory*. It is omitted from other examples in order to keep them minimal.
...
@RegisterDataController()
export class MyDataAPI extends BaseDataAPI.extend('/subtable') {
