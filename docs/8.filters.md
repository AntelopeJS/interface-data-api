...
```typescript
function filterGmailAccounts(
  context: RequestContext,
  proxy: ValueProxy.Proxy<any>, // alias of row(key)
  key: string,
  value: FilterValue[0],        // string
  mode: FilterValue[1],         // Comparison
  row: ValueProxy.Proxy<U>
) {
  return proxy.match('@gmail\\.com$');
}
...
- **`proxy`**: A ValueProxy object representing the field value with modifier transformations already applied (e.g., decrypted, localized). This is the unlocked value, unlike `row(key)` which would return the raw protected value.
- **`key`**: The field name being filtered
- **`value`**: The filter value from the query parameter
- **`mode`**: The comparison mode ('eq', 'gt', 'lt', ...)
- **`row`**: The database row object being filtered
