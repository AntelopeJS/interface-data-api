...
- `noMandatory`: Disables mandatory field validation
- `pluckMode`: Sets default field selection mode for list operations 

## Custom Route Implementation

For advanced use cases, you can implement custom routes:

# TODO: NOT SURE ABOUT THIS PART, NEEDS REVIEW
```typescript
import { DataController, RegisterDataController } from '@ajs/data-api/beta';
...
const customRoutes = {
  ...
  search: {
    ...
    args: [/* parameter decorators */],
    method: 'GET'
  }
};

@RegisterDataController()
class CustomUserAPI extends DataController(User, customRoutes, Controller('/users')) {
  // Field definitions...
}
```
